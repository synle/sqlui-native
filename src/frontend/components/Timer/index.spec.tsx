...
describe("Timer", () => {
  ...
  test("renders duration when both startTime and endTime are provided", () => {
    ...
    expect(container.textContent).toEqual("5s");
  });

  test("renders '<= 1s' for short durations", () => {
    const now = Date.now();
    const { container } = render(<Timer startTime={now} endTime={now + 500} />);
    expect(container.textContent).toEqual("<= 1s");
  });
  ...
  test("updates display as time passes when endTime is not set", () => {
    ...
    expect(container.textContent).toEqual("3s");
  });
