...
app.get('/api/connection/:connectionId/databases', async (req, res) => {
  ...
  const engine = new RelationalDatabaseEngine(connection.connection);
...
app.get('/api/connection/:connectionId/database/:databaseId/table/:tableId/columns', async (req, res) => {
  const connection = await ConnectionUtils.getConnection(req.params?.connectionId);
  const engine = new RelationalDatabaseEngine(connection.connection);
